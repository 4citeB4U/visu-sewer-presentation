...
WHERE=tools/leeway_unified_audit.js; WHEN=2025-10-04;
HOW=Node/JS, --strict flags, asset discovery, id lines, --report-html
...
const BACKEND_REQUIRED_DIRS = [
  "agentlee_mcp_hub/mcp","checkpoints/converter","checkpoints/base_speakers/EN","checkpoints/base_speakers/ZH",
  "models","routes","scripts","tools","webrtc","run",
];
const REQUIRED_PY_PKGS = ["fastapi","uvicorn","structlog","pydantic","pydantic-settings","websockets","soundfile","scipy","numpy","ffmpeg-python","jieba"];
...
function scanDuplicateIds(root){
  ...
  for(const f of files){
    ...
    while((m=DUPLICATE_ID_RE.exec(t))){
      const id=m[1]; lc[id]=(lc[id]??0)+1; const line=t.slice(0,m.index).split("\n").length; (ll[id] ||= []).push(line);
    }
    const offenders=Object.fromEntries(Object.entries(lc).filter(([,c])=>c>1));
    if(Object.keys(offenders).length){counts[f]=offenders; lines[f]=Object.fromEntries(Object.keys(offenders).map(k=>[k,ll[k]||[]]));}
  }
...
function checkAssets(publicDir, rels){const res={}; for(const r of rels) res[r]=exists(path.join(publicDir,r)); return res;}
...
function checkBackendDirs(root){
  const res={};
  for(const rel of BACKEND_REQUIRED_DIRS){ res[rel] = exists(path.join(root, ...rel.split("/"))); }
  return res;
}
...
function checkBackendRequirements(root){
  const p=path.join(root,"requirements.txt"); let text=""; try{text=fs.readFileSync(p,"utf8");}catch{}
  const present=[], missing=[]; for(const pkg of REQUIRED_PY_PKGS){const rx=new RegExp(`^${pkg}(\\W|$)`,"im"); if(rx.test(text)) present.push(pkg); else missing.push(pkg);} return {present,missing};
}
...
function run(){
  ...
  if(mode==="frontend"||mode==="all"){out.frontend=auditFrontend(FRONTEND_ROOT);writeJSON(path.join(FRONTEND_ROOT,"run","leeway_frontend_audit_report.json"),out.frontend);}
  if(mode==="backend"||mode==="all"){out.backend=auditBackend(BACKEND_ROOT);writeJSON(path.join(BACKEND_ROOT,"run","leeway_audit_report.json"),out.backend);}

  out._ignoreFfmpeg=ignoreFfmpeg;
  if(reportHtml){const outHtml=path.join(BACKEND_ROOT,"run","leeway_audit_report.html");writeHtmlReport(outHtml,out.frontend,out.backend);}
