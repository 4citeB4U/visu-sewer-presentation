  ...
  return (
    <div className="mt-2 rounded bg-slate-50 border border-slate-200 p-2">
      ...
      <ul className="flex flex-wrap gap-2">
        {webSources.map((src: WebSource) => (
          <li key={src.id}>
            <a href={src.url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-blue-700 underline decoration-dotted hover:text-blue-900">
              {src.label}
            </a>
          </li>
        ))}
